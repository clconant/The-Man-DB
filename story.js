...
const OPENING_SCENE_ID = 'receGixAbZWOyQRFn';
...
function getScene(record_id) {
  ...
  const key = 'keyX7wmnGhRRv8Zgv';
  const base_url = 'appcoxBsNOIDdj5gm'; 
    ...
    .done(function (data) {
      ...
      if (data.fields.choices) {
        ...
        for (let idx = 0; idx < data.fields.choices.length; idx++) {
          choices.push($.ajax({
            url: `https://api.airtable.com/v0/appcoxBsNOIDdj5gm/choices/${data.fields.choices[idx]}?api_key=${key}`,
            type: 'GET'
          }));
        }
